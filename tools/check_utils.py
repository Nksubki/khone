...
print("۸) امتیاز گرد بودن مبلغ (برای انتخاب بهترین حدس صوتی)")
check(N.roundness_score(100000000) > N.roundness_score(100000123), "roundness ترتیب")
check(N.roundness_score(0) == 0, "roundness صفر")

print("۹) آیکن‌ها")
from ledger.icons import CATEGORY_ICONS, ICON_KEYS, safe_icon  # noqa: E402

check(safe_icon("rebar") == "rebar", "safe_icon کلید معتبر")
check(safe_icon("🧱") == "box", "safe_icon ایموجی قدیمی → پیش‌فرض")
check(safe_icon(None) == "box", "safe_icon خالی")
check(len(CATEGORY_ICONS) >= 30, "تعداد آیکن دسته‌بندی")
check(len(ICON_KEYS) >= 55, "تعداد کل آیکن‌ها: %d" % len(ICON_KEYS))
